...
func buildRaxPrimarySearchResponse(ctx context.Context, eng yeoul.Engine, req yeoul.SearchRequest, docIDs []string) (*yeoul.SearchResponse, error) {
	...
	cursor, err := decodeRaxCursor(req.Page.Cursor)
	if err != nil {
		return nil, err
	}
	...
	for rank, docID := range docIDs {
		...
		if coreMatched {
			score = 1 + coreScore + 1/float64(rank+1)
			reasons = append(reasons, fmt.Sprintf("core_rerank_score:%.6f", coreScore))
		} else {
			reasons = append(reasons, "current_text_match")
		}
		hit := yeoul.SearchHit{
			HitID:       "hit_" + id,
			HitType:     kind,
		...
		candidates = append(candidates, raxCandidate{hit: hit, record: record.Record})
	}
	sort.SliceStable(candidates, func(i, j int) bool {
		if candidates[i].hit.Score != candidates[j].hit.Score {
			return candidates[i].hit.Score > candidates[j].hit.Score
		}
		if candidates[i].hit.HitType != candidates[j].hit.HitType {
			return candidates[i].hit.HitType < candidates[j].hit.HitType
		}
		return candidates[i].hit.RecordID < candidates[j].hit.RecordID
	})
	nextCursor := ""
	start := raxCursorStart(candidates, cursor)
	if start < 0 {
		return nil, fmt.Errorf("cursor_invalid: %s", req.Page.Cursor)
	}
	end := start + limit
	if end < len(candidates) {
		nextCursor = encodeRaxCursor(candidates[end-1].hit)
	} else {
		end = len(candidates)
	}
...
func decodeRaxCursor(cursor string) (raxCursor, error) {
	if strings.TrimSpace(cursor) == "" {
		return raxCursor{}, nil
	}
	if !strings.HasPrefix(cursor, "key:") {
		return raxCursor{}, fmt.Errorf("cursor_invalid: %s", cursor)
	}
	parts := strings.SplitN(strings.TrimPrefix(cursor, "key:"), ":", 3)
	if len(parts) != 3 {
		return raxCursor{}, fmt.Errorf("cursor_invalid: %s", cursor)
	}
	...
	return raxCursor{score: score, hitType: parts[1], recordID: parts[2]}, nil
}

func encodeRaxCursor(hit yeoul.SearchHit) string {
	return fmt.Sprintf("key:%.17g:%s:%s", hit.Score, hit.HitType, hit.RecordID)
}

func raxCursorStart(candidates []raxCandidate, cursor raxCursor) int {
	if cursor.recordID == "" {
		return 0
	}
	for i, candidate := range candidates {
		hit := candidate.hit
		if hit.Score == cursor.score && hit.HitType == cursor.hitType && hit.RecordID == cursor.recordID {
			return i + 1
		}
	}
	return -1
