...
// projectionRecordID builds the stable projection document identity for a
// record as "kind:id".
func projectionRecordID(kind, id string) string {
	return kind + ":" + id
}
...
func buildProjectionArtifacts(dbPath string, payload *exportFile) ([]projectionDocument, projectionManifest) {
	...
	for _, episode := range payload.Episodes {
		...
		doc := projectionDocument{
			ProjectionID: projectionRecordID("episode", episode.ID),
	...
	for _, entity := range payload.Entities {
		...
		projections = append(projections, projectionDocument{
			ProjectionID: projectionRecordID("entity", entity.ID),
			SearchText:   text,
			Metadata:     meta,
		})
	}
	...
	for _, fact := range payload.Facts {
		...
		doc := projectionDocument{
			ProjectionID: projectionRecordID("fact", fact.ID),
			SearchText:   text,
			Metadata:     meta,
		}
		if !fact.ObservedAt.IsZero() {
