...
// ensurePrivateFileSupported refuses exports on macOS: the platform can grant
...
func ensurePrivateFileSupported() error {
	return fmt.Errorf("admin export is not supported on macOS because a private export file cannot be created atomically; run the export on Linux")
}
