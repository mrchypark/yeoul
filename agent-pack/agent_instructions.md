...
4. For an authorized confirmed durable claim, do not stop after episode ingest because an entity is missing. Select namespace, type, canonical name, and stable key; run `yeoul entity resolve` to check whether an entity already matches the identity tuple and reuse its ID, or assert with `--upsert-subject`. For example: `yeoul entity resolve --db memory.ltdb --namespace people --type Person --name Alex --stable-key alex-123`. Use `--upsert-object` for a reusable named relationship target and `--value-text` for scalar or opaque text. `--upsert-subject` and `--upsert-object` fail closed with `YEOUL_ENTITY_NEAR_DUPLICATE` when the derived entity ID is new but an existing entity already matches the same identity under a different ID, including when the display name differs only by case. The guard is conservative: only an exact same-scope legacy ID may be reused automatically; ambiguous key/name or namespace/type drift requires an explicit ID.
