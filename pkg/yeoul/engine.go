...
func (e *engine) IngestBatch(ctx context.Context, input BatchInput) (*BatchResult, error) {
	...
	for _, fact := range input.Facts {
		...
		item, err := e.assertFactLocked(normalizeSpaceID(fact.SpaceID), fact, input.AllowLifecycleFields)
		if err != nil {
			restore()
			return nil, err
		}
	...
	if len(input.EntityRevisions) > 0 || len(input.FactRevisions) > 0 {
		if !input.AllowLifecycleFields {
			restore()
			return nil, errorf(ErrLifecycleInvalid, "revision import requires lifecycle restore mode", nil, nil)
		}
		for _, revision := range input.EntityRevisions {
			if strings.TrimSpace(revision.ID) == "" || strings.TrimSpace(revision.EntityID) == "" {
				restore()
				return nil, errorf(ErrInputInvalid, "entity revision id and entity_id are required", map[string]any{"revision_id": revision.ID}, nil)
			}
			e.entityRevisions[revision.ID] = *cloneEntityRevision(revision)
		}
		for _, revision := range input.FactRevisions {
			if strings.TrimSpace(revision.ID) == "" || strings.TrimSpace(revision.FactID) == "" {
				restore()
				return nil, errorf(ErrInputInvalid, "fact revision id and fact_id are required", map[string]any{"revision_id": revision.ID}, nil)
			}
			e.factRevisions[revision.ID] = *cloneFactRevision(revision)
		}
	}
	if err := e.saveLocked(); err != nil {
		restore()
	...
	if slices.Contains(types, "fact") {
		for _, fact := range e.facts {
			...
			if matched {
				...
				e.addFactSupport(&included, *factRecord, req.Scope)
			}
		}
	}
...
func (e *engine) addFactSupport(included *IncludedRecords, fact Fact, scope ScopeFilter) {
	if entity, ok := e.entities[fact.SubjectID]; ok && matchesEntityType(entity, scope.EntityTypes) {
		included.Entities = append(included.Entities, entity)
	}
	if fact.ObjectID != "" {
		if entity, ok := e.entities[fact.ObjectID]; ok && matchesEntityType(entity, scope.EntityTypes) {
			included.Entities = append(included.Entities, entity)
		}
	}
	for _, episodeID := range fact.SupportingEpisodeIDs {
		if episode, ok := e.episodes[episodeID]; ok && matchesScopeForEpisode(episode, scope, e.sources) {
			included.Episodes = append(included.Episodes, episode)
			if source, ok := e.sources[episode.SourceID]; ok {
				included.Sources = append(included.Sources, source)
			}
...
func matchesScopeForEpisode(episode Episode, scope ScopeFilter, sources map[string]Source) bool {
	...
	if len(scope.SourceKinds) > 0 {
		...
		if !ok || !slices.Contains(scope.SourceKinds, source.Kind) {
			return false
		}
	}
	return true
...
func RecordPassesSearchFilters(ctx context.Context, eng Engine, record any, req SearchRequest) bool {
	...
	case *Fact:
		...
		if len(req.Scope.EntityTypes) > 0 {
			subject, subjectErr := eng.GetRecord(ctx, GetRecordRequest{Kind: "entity", ID: value.SubjectID, Temporal: req.Temporal})
			object, objectErr := eng.GetRecord(ctx, GetRecordRequest{Kind: "entity", ID: value.ObjectID, Temporal: req.Temporal})
		...
		for _, episodeID := range value.SupportingEpisodeIDs {
			...
			if err == nil && RecordPassesSearchFilters(ctx, eng, episode, SearchRequest{Scope: req.Scope}) {
				return true
			}
		}
		return false
	case *Episode:
		...
		if len(req.Scope.SourceKinds) > 0 {
			source, err := eng.GetSource(ctx, value.SourceID)
			if err != nil || !slices.Contains(req.Scope.SourceKinds, source.Kind) {
				return false
			}
		}
		return true
